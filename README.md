...
The installer uses the pinned Docker image tag defined in `kvs-conversion-server.sh` by default.
...
Keep `.kvs-server.conf` private. It contains the FTP password and is written with `600` permissions. If you need the password later, read `FTP_PASS` from this file or run the script with `info --show-password`.
...
| `--php-version VERSION` | Supported PHP CLI version. Omit to use the default. | Installer default |
| `--ftp-mode MODE` | FTP mode: `ftp`, `ftps`, `ftps_implicit`, or `ftps_tls`. | `ftp` |
...
| `--ipv4 ADDRESS` | Public IPv4 address used for passive FTP. Must not be `0.0.0.0`. | Auto-detected |
| `--cpu-limit CORES` | Docker CPU limit. Decimal values are accepted. | All cores |
| `--num-folders NUMBER` | Number of KVS working directories to create. | Installer default |
| `--image-tag TAG` | Docker image tag to install and save in `.kvs-server.conf`. | Installer default |
| `--auto-stop-container` | Stop and replace an existing `conversion-server` container during installation. | Disabled |
...
| `KVS_PHP_VERSION=VERSION` | `--php-version VERSION` |
...
| `KVS_IMAGE_TAG=TAG` | `--image-tag TAG` |
| `KVS_AUTO_STOP_CONTAINER=true` | `--auto-stop-container` |

CLI options take precedence over environment variables.

Run the script with `--help` to show current defaults and supported values.

Omit `KVS_FTP_PASS` in headless mode to generate a password automatically.
Use `--ftp-pass=PASSWORD` when a password starts with a dash.
...
| `./kvs-conversion-server.sh info` | Show saved configuration and live container details with the FTP password masked. |
| `./kvs-conversion-server.sh info --show-password` | Show saved configuration and reveal the saved FTP password. |
...
Headless remove is intentionally explicit. Run `KVS_CONFIRM_REMOVE=true ./kvs-conversion-server.sh --headless remove` to remove the container without prompts. It keeps local data and config by default; add `KVS_REMOVE_DATA=true` or `KVS_REMOVE_CONFIG=true` only when those files should also be deleted.
...
| --- | --- |
| Connection type | FTP |
| Force SSL connection | `true` for `ftps`, `ftps_tls`, and `ftps_implicit`, otherwise `false` |
| FTP host | Public IPv4 address detected or passed with `--ipv4` |
| FTP port | `21` for `ftp`, `ftps`, and `ftps_tls`; `990` for `ftps_implicit` |
| FTP user | Value passed with `--ftp-user` |
| FTP password | Value passed with `--ftp-pass` or generated by the installer. Recover it later with `info --show-password` or `FTP_PASS` in `.kvs-server.conf`. |
...
- VsFTPd 3.0.5
- PHP CLI versions required for supported KVS compatibility targets
- IonCube Loader for the supported PHP versions
...
Docker image tags omit the `v` prefix. A release tag publishes:

- `maximemichaud/kvs-conversion-server:MAJOR.MINOR.PATCH`
- `maximemichaud/kvs-conversion-server:MAJOR.MINOR`
- `maximemichaud/kvs-conversion-server:latest`

The installer defaults to the pinned tag in `DEFAULT_IMAGE_TAG`. Use `--image-tag latest` only when you intentionally want the moving latest image.
...
Each folder gets a cron entry that runs `remote_cron.php` when KVS uploads it. Cron task output is truncated before execution when the per-folder log reaches 10 MiB.
...
shellcheck --severity=warning kvs-conversion-server.sh scripts/create_folders.sh scripts/run-cron-task.sh scripts/run-vsftpd.sh scripts/entrypoint.sh
hadolint Dockerfile
...
```bash
VERSION=$(awk -F= '/^DEFAULT_IMAGE_TAG=/ { gsub(/"/, "", $2); print $2 }' kvs-conversion-server.sh)
docker build \
...
```bash
PHP_VERSION=$(awk -F= '/^SUPPORTED_PHP_VERSIONS=/ { gsub(/"/, "", $2); count = split($2, versions, " "); print versions[count] }' kvs-conversion-server.sh)
docker run --rm -d \
  --name kvs-test \
  -e FTP_USER=testuser \
